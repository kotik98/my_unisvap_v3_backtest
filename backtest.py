...
def calcFees(data, pool, priceToken, liquidity, unboundedLiquidity, investment, min, max):
    ...
    for i in range(len(data)):
        ...
        latest_TVL_USD = float(ast.literal_eval(data["pool"].values[-1])["totalValueLockedUSD"])
        latest_TVL0 = float(ast.literal_eval(data["pool"].values[-1])["totalValueLockedToken0"])
        latest_TVL1 = float(ast.literal_eval(data["pool"].values[-1])["totalValueLockedToken1"])
